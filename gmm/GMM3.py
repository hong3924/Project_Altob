...
LastEditTime: 2022-05-16 06:58:05
...
# Output directions 
bg_out = cv2.VideoWriter(file_name.split('/')[-1].split('.')[0] + '_' + 'background_3' + '.mp4', fourcc, 20, (width, high))
fg_out = cv2.VideoWriter(file_name.split('/')[-1].split('.')[0] + '_' + 'foreground_3' + '.mp4', fourcc, 20, (width, high))
fgmask_out = cv2.VideoWriter(file_name.split('/')[-1].split('.')[0] + '_' + 'fgmask_3' + '.mp4', fourcc, 20, (width, high), 0)
new_fgmask_out = cv2.VideoWriter(file_name.split('/')[-1].split('.')[0] + '_' + 'new_fgmask_3' + '.mp4', fourcc, 20, (width, high), 0)

# MOG2
fgbg = cv2.createBackgroundSubtractorMOG2(history=500, varThreshold=200, detectShadows=False)# varThreshold=1000

# Posprocess for fgmask
def posprocess(img):
    #img = cv2.dilate(img, None, iterations=2)
    #img = cv2.medianBlur(img, 3)
    #img = cv2.dilate(img, None, iterations=20)
    #img = cv2.erode(img, None, iterations=20)
...
def fill_area(noise, fgobj, fgmask):
    ...
    # Fill foreground obj with white
    cv2.drawContours(fgmask, fgobj, -1, (255,255,255), thickness=-1)

    '''
    # Fill foreground obj with rectangle
    for c in fgobj:
    ...
    for c in fgobj:
        ...
        box = np.int(box)
        cv2.drawContours(fgmask, [box], -1, (255,255,255), thickness=-1)
    '''
...
while True:
    ...
    #############################################################
    new_fgmask = cv2.dilate(fgmask, None, iterations=3)
    #new_fgmask = fgmask
    ...
    #new_fgmask = cv2.erode(new_fgmask, None, iterations=4)
    
    #############################################################
    
    new_fgmask = cv2.dilate(new_fgmask, None, iterations=6)
    cnts = findContours(new_fgmask)
    minArea = 40000
    c_noise = []
    c_fgobj = []
    ...
    new_fgmask = fill_area(c_noise, c_fgobj, new_fgmask)
    #new_fgmask = cv2.erode(new_fgmask, None, iterations=5)
